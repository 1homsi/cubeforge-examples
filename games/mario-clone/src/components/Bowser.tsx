...
function bowserUpdate(id: EntityId, world: ECSWorld, _input: unknown, dt: number) {
  ...
  for (const [fid, fdata] of flames) {
    ...
    if (pid) {
      ...
      if (pt && Math.abs(ft.x - pt.x) < 28 && Math.abs(ft.y - pt.y) < 24) {
...
export function Bowser({ x, y, patrolLeft, patrolRight }: BowserProps) {
  ...
  return (
    <Entity tags={['enemy', 'bowser']}>
      <Transform x={x} y={y} />
      <Sprite src="/SMB_Bowser_Sprite.png" width={64} height={64} color="#ff6f00" zIndex={10} />
      <RigidBody friction={1} />
      <BoxCollider width={60} height={60} mask="world" />
      <Script
        init={(id, world) => {
